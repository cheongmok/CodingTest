...
public class SWEA5215 {
	...
	static void dfs(List<Ingredient> list, int curIndex, int kcalSum, int scoreSum, int ingredients, int limitKcal) {
		// kcal 제한 넘으면 탐색 종료
		...
		// 현재 재료를 넣고 dfs
		dfs(list, curIndex+1, kcalSum + list.get(curIndex).kcal, scoreSum + list.get(curIndex).score, ingredients, limitKcal);
		// 현재 재료를 안넣고 dfs
		dfs(list, curIndex+1, kcalSum, scoreSum, ingredients, limitKcal);
	}
	...
	public static void main(String[] args) {
		...
		for (int testCase = 1; testCase <= totalCase; ++testCase) {
			...
			dfs(ingredientList, 0, 0, 0, ingredients, limitKcal);
